...
#             cursor.execute('''
#                 INSERT INTO dim_date (date_key, full_date, year, quarter, quarter_name, month, month_name, week_of_year, week_start_date, week_end_date)
#                 VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
#                 ON CONFLICT (date_key) DO NOTHING
#             ''', (date['date_key'], date['full_date'], date['year'], date['quarter'],
#                   date['quarter_name'], date['month'], date['month_name'], date['week_of_year'],
#                   date['week_start_date'], date['week_end_date']))

#         # Load dim_publisher with auto-incremented surrogate key
#         for publisher in transformed_data['publishers']:
#             cursor.execute('''
#                 INSERT INTO dim_publisher (publisher_name, effective_start_date, is_current)
#                 VALUES (%s, NOW(), TRUE)
...
#             cursor.execute('''
#                 INSERT INTO dim_list (list_id, list_name, display_name, update_frequency, list_image_url)
#                 VALUES (%s, %s, %s, %s, %s)
#                 ON CONFLICT (list_id) DO NOTHING
#             ''', (book_list['list_id'], book_list['list_name'], book_list['display_name'],
#                   book_list['update_frequency'], book_list['list_image']))

#         # Load dim_book
#         for book in transformed_data['books']:
#             cursor.execute('''
#                 INSERT INTO dim_book (title, author, publisher, primary_isbn13, primary_isbn10, description, created_date, is_current)
#                 VALUES (%s, %s, %s, %s, %s, %s, NOW(), TRUE)
#                 ON CONFLICT (primary_isbn13) DO NOTHING
#             ''', (book['title'], book['author'], book['publisher'],
#                   book['isbn13'], book['isbn10'], book['description']))

#         # Load fact_book_rankings
#         for ranking in transformed_data['rankings']:
#             cursor.execute('''
#                 INSERT INTO fact_book_rankings (date_key, book_key, list_key, publisher_key, rank, published_date)
#                 SELECT d.date_key, b.book_key, l.list_key, p.publisher_key, %s, %s
#                 FROM dim_book b
#                 JOIN dim_list l ON l.list_id = %s
#                 JOIN dim_publisher p ON p.publisher_name = b.publisher
#                 JOIN dim_date d ON d.full_date = %s
#                 WHERE b.primary_isbn13 = %s
#             ''', (ranking['rank'], ranking['published_date'],
#                   ranking['list_id'], ranking['published_date'], ranking['isbn13']))

#         # Load fact_publisher_performance
#         cursor.execute('''
#             INSERT INTO fact_publisher_performance (publisher_key, date_key, quarter, year, total_points, books_in_top_5, rank_1_count, rank_2_count, rank_3_count, rank_4_count, rank_5_count)
#             SELECT p.publisher_key, d.date_key, d.quarter, d.year,
...
#             JOIN dim_publisher p ON f.publisher_key = p.publisher_key
#             JOIN dim_date d ON f.date_key = d.date_key
#             GROUP BY p.publisher_key, d.date_key, d.quarter, d.year
#         ''')
